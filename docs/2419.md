...
执行安装后，默认情况下，您的沙盒实例将能够一次处理一个样本。执行实际分析的服务称为`drakrun@<instance_number>`。您可以通过执行以下命令来检查特定实例的状态:
...
DRAKVUF 沙盒是基于 karton 框架的，`<object>`组件交互的高级视图。建议在接触沙盒代码之前熟悉它的概念。
...
[**Download**](https://github.com/CERT-Polska/drakvuf-sandbox)